...
详情参见./video下的视频，由于已经过了课堂验收，视频比较简单。
...
== 用户账户管理
用鼠标点击导航栏右上角的用户图标，会弹出一个下拉菜单，可以选择注销退出登录，也可以选择进入用户设置界面

#figure(
  image("./assets/manual/user_select.png", width: 80%)
)

=== 退出登录
点击其中的退出登录按钮，即可退出登录，网站会自动跳转到登录界面。

=== 账户设置
点击Settings选项，则可以跳转到用户信息设置界面，在该页面中，可以修改自己的用户信息，包括用户名、密码、邮箱等。

#figure(
  image("./assets/manual/user_setting.png", width: 80%)
)

在该页面中，上方展示了用户的基本信息，包括用户名、邮箱，在页面左侧可以选择管理账户或修改密码。点击账户菜单，可以修改用户名和邮箱，点击密码菜单，可以修改密码，点击右上角的返回按钮可以返回到搜索界面。

==== 修改用户名和邮箱
上图已经显示了账户菜单，现在只需要在对应的输入框中输入新的用户名和邮箱，点击修改按钮即可保存修改，修改成功会弹出对话框提示成功。

==== 删除账户：用户可以删除自己的账户
点击删除账户按钮，会弹出一个确认删除的对话框，输入密码之后点击下方的“我确定要删除我的账户！”按钮即可删除账户。删除成功后会跳转到登录界面

#figure(
  image("./assets/manual/delete_account.png", width: 50%)
)

==== 修改密码
点击密码菜单，可以跳转到密码管理界面。在对应的输入框中输入新的密码，点击修改按钮即可保存修改，修改成功会弹出对话框提示成功。

#figure(
  image("./assets/manual/change_password.png", width: 80%)
)
